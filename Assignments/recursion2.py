...
#  Description: string recursion examples
...
def changePi(str):
  if(str == ""):
  	...
  	if(str[0:2] == "pi"):
  	  ...
  	  return str[0] + "3.14" + changePi(str[3:])
  	else:
...
def strCount(str, sub):
  ...
  if (len(str) < sub_len):
    return 0
  	...
  	if(str[:sub_len] == sub):
  	  return 1 + strCount(str[sub_len:], sub)
  	else:
...
def strDist(str, sub):
  ...
  if(len(str) < sub_len):
    return 0
  elif(str[:sub_len] == sub and str[-sub_len:] == sub):
  	return len(str)
  else:
  	if(str[:sub_len] == sub):
  	  return strDist(str[:-1], sub)
  	elif(str[-sub_len:] == sub):
  	  return strDist(str[1:], sub)
  	else:
