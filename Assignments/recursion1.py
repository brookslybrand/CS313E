...
def bunnyEars2(bunnies):
  # if no bunnies, 0 ears
  if (bunnies == 0):
    return 0
  # cases for even bunnies
  elif (bunnies == -1):
    return 1
  else:
  # count bunnies before, and find out if there are any extra ears
    return bunnyEars2(bunnies - 1) + bunnyEars2((bunnies % 2) - 1) + 2
...
def count7(n):
  # check for if it's a 7
  if(n == -1):
    return 1
  # check if it's zero or less
  elif (n <= 0):
    return 0
  # recursively itterate over the remaining digits and also check if last digit is a 7
  else:
    return count7(n // 10) + count7((n % 10) - 8)
...
# while divide (//) by 10 removes the rightmost digit (126 // 10 is 12).
def count8(n):
  # check for if it's an 8
  if(n == -1):
    return 1
  # check if it's zero or less
  elif (n <= 0):
    return 0
  # recursively itterate over the remaining digits and also check if last digit is a 8
  # or if there are two eights by each other
  else:
    return count8(n // 10) + count8((n % 10) - 9) + count8((n % 100) - 89)
