...
#  Description: Recursion examples with lists
...
def groupSum6(start, nums, target):
  if(target == 0 and not (6 in nums[start:])):
  	return True
  if(start == len(nums)):
  	return False
  else:
  	if(nums[start] == target and target == 6):
  	  return True
  	else:
  	  if (nums[start] == 6):
...
def groupSum5(start, nums, target):
  if(target == sum(nums) and start == len(nums)):
    return True
  if(start >= len(nums)):
    return False
  else:
  	if(nums[start] % 5 == 0):
  	  ...
  	  nums_rm = nums[:]
  	  nums_rm.remove(nums[start])
  	  return groupSum5(start, nums_rm, target) or groupSum5(start + 1, nums, target)
...
def groupSumClump(start, nums, target):
  ...
  if(start == len(nums)):
  	...
  	repeats = True
  	count = 1
  	while(repeats and (count + start) < len(nums)):
  	  if(n == nums[start+count]):
  	  	count += 1
  	  else:
  	  	repeats = False
  	if(n == target and count == 1):
  	  return True
  	else:
  	  return groupSumClump(start+count, nums, target) or groupSumClump(start+count, nums, target - count*n)
...
def splitOdd10Helper(a, b, idx):
  if(idx == len(a)):
  	return (sum(b)%10 == 0) and ( (sum(a) - sum(b))%2 == 1 )
  else:
  	c = b[:]
  	b.append(a[idx])
  	return splitOdd10Helper(a, b, idx+1)or splitOdd10Helper(a, c, idx+1)
...
def split53Helper(a, b, idx, sum_type):
  if(idx == len(a)):
  	return sum(b) == sum(a) - sum(b)
  else:
    num = a[idx]
    if(sum_type == 5):
      if(num%5 == 0):
        b.append(num)
        return split53Helper(a, b, idx+1, sum_type)
      elif(num%3 == 0):
        return split53Helper(a, b, idx+1, sum_type)
      else:
        c = b[:]
        b.append(num)
        return split53Helper(a, b, idx+1, sum_type) or split53Helper(a, c, idx+1, sum_type)
    elif(sum_type == 3):
      if(num%3 == 0):
        b.append(num)
        return split53Helper(a, b, idx+1, sum_type)
      ...
      if(num%5 == 0):
      	c = b[:]
      	b.append(num)
      	return split53Helper(a, b, idx+1, 5) or split53Helper(a, c, idx+1, 3)
      elif(num%3 == 0):
      	c = b[:]
      	b.append(num)
      	return split53Helper(a, b, idx+1, 3) or split53Helper(a, c, idx+1, 5)
      else:
      	c = b[:]
      	b.append(num)
      	return split53Helper(a, b, idx+1, 0) or split53Helper(a, c, idx+1, 0)
